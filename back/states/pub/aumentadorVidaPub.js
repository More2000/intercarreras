...
const topic = "aumentadorVida"; // Cambia este tópico a "necesidades"
...
app.post("/aumentadorVida/publish", (req, res) => {
  const { foodAmount, waterAmount } = req.body;

  if (foodAmount === undefined || waterAmount === undefined) {
    return res
      .status(400)
  ...
  client.publish(topic, message, (err) => {
    ...
    console.log(`Datos de aumentar vida publicados: ${message}`);
    res
      .status(200)
      .send(
        `Datos de aumentar vida publicados en el tópico ${topic}: ${message}`
      );
  });
});
